...
void program(){
    ...
    map<Vertex, unordered_sets<Edge>> vertex_to_edges;
    ...
    if (file.is_open()) {
        ...
        getline(file, line);
        int i = line.find(" ");
        int num_vertex = stoi(line.subr(0, i).c_str());
        int num_edges = stoi(line.subr(i+1).c_str());
        while (getline(file, line)) {
            i = line.find(" ");
            if (already_handled < num_vertex) {
                int id = stoi(line.subr(0, i).c_str());
                int weight = stoi(line.subr(i+1).c_str());
                Vertex v;
                v.id = id;
                v.weight = weight;
                v.score = 0.0;
                ...
                int id1 = stoi(line.subr(0, i).c_str());
                int id2 = stoi(line.subr(i+1).c_str());
                Edge edge;
                edge.v1 = id_to_vertex[id1];
                edge.v2 = id_to_vertex[id2];
                available_edges.insert(edge);
                vertex_to_edges[v1].push_back(edge);
                vertex_to_edges[v2].push_back(edge);
            }
            already_handled++;
        }
        file.close();
